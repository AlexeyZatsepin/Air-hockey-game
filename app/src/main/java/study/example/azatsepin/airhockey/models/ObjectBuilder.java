...
import study.example.azatsepin.airhockey.utils.geometry.Circle;
import study.example.azatsepin.airhockey.utils.geometry.Cylinder;
...
// the point at the starting angle twice to complete the fan.
        for (int i = 0; i <= numPoints; i++) {
            ...
            vertexData[offset++] =
                    circle.center.x
                            + circle.radius * (float)cos(angleInRadians);
            vertexData[offset++] = circle.center.y;
            vertexData[offset++] =
                    circle.center.z
                            + circle.radius * (float)sin(angleInRadians);
        }
    ...
    private void appendOpenCylinder(Cylinder cylinder, int numPoints) {
        ...
        for (int i = 0; i <= numPoints; i++) {
            ...
            float xPosition =
                    cylinder.center.x
                            + cylinder.radius * (float) cos(angleInRadians);
            float zPosition =
                    cylinder.center.z
                            + cylinder.radius * (float) sin(angleInRadians);
